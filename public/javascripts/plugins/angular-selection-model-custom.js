/*
 SE CUSTOMIZO QUE CHECKEE EL ESTADO DEL OBJETO SELECCIONADO Y QUE SI NO ESTA EN FINISHED QUE NO LO SELECCIONE
*/
/**
  ...
  function(selectionStack, uuidGen, selectionModelOptions) {
    ...
    return {
      ...
      link: function(scope, element, attrs) {
         ...
         */
        var repeatLine = attrs.ngRepeat || attrs.dirPaginate;
        if(!repeatLine) {
        ...
        var handleClick = function(event) {
          if(smItem && smItem.status){
            if(smItem.status != "finished"){
              return
            }
          }else{
            if(smItem && ((smItem.action && smItem.action.status != 'error') || smItem.deployment)){
              return
            }
          }
